...
namespace LEO
{
	SubModel::SubModel(MeshRef& mesh, Material& mat, Transform& local)
		:
		mesh(mesh),
		material(mat),
	...
	u32 Model::NumberOfSubModels() const
	{
		return (u32)m_sub_models.size();
	}

	void Model::AddSubModel(MeshRef& mesh, Material& mat, Transform& local)
	{
		m_sub_models.emplace_back(mesh, mat, local);
	}
		...
		SetExtraUniforms setExtraUniforms)
	{
		...
		for (i32 i = 0; i < m_sub_models.size(); i++)
		{
			...
			Mesh& mesh = m_sub_models[i].mesh.Get();
